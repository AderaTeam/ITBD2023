...
async def create_item(item: Data):
    ...
    for i in item.data:
        ...
        theme_group = theme_group_nums[torch.argmax(model_theme_group(v.to('cuda').double())[0]).item()]
        theme = themes_num[str(torch.argmax(model_theme(v.to('cuda').double())[0]).item())]
        res['data'][i] = {
